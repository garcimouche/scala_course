...
import nodescala.NodeScala.Default
...
object Main {
  ...
  def main(args: Array[String]) {
    ...
    val myServer: Default = new NodeScala.Default(8191)
    ...
    terminationRequested onSuccess {
      case msg => {println("server stops with message " + msg);myServerSubscription.unsubscribe()}
    }
  }
