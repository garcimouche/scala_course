...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    val ls = createListener(relativePath)
    val sls = ls.start()
    println("starting server")
    val subscription = Future.run()(cToken =>
      async {
        while (cToken.nonCancelled) {
          println("waiting for request....")
          val res: (Request, Exchange) = await(ls.nextRequest())
          respond(res._2, cToken, handler(res._1))
        }
      }
    )
    Subscription(sls,subscription)
  }
