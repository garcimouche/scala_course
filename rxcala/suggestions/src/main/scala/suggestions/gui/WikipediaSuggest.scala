  ...
  def top = new MainFrame {
    ...
    val suggestions: Observable[Try[List[String]]] =
      searchTerms.concatRecovered(
        term => Observable.from(wikipediaSuggestion(term))
      )

    // TO IMPLEMENT
    val suggestionSubscription: Subscription = suggestions.observeOn(eventScheduler) subscribe {
      t => t match {
        case Success(ls) => suggestionList.listData = ls
    ...
    val selections: Observable[String] =
      button.clicks.flatMap(_ => Observable.from(suggestionList.selection.items))

    // TO IMPLEMENT
    val pages: Observable[Try[String]] =
      selections.concatRecovered(
        sel => Observable.from(wikipediaPage(sel))
      )
