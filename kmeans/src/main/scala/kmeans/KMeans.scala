...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val byM = points.groupBy { p => findClosest(p, means) }
    val emptyM = 
      for(k <- means;if(byM.get(k) == None))
        yield( (k,GenSeq.empty) )
    byM.++(emptyM)
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] =  
    oldMeans.map( op => findAverage(op,classified(op)) )

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zipWithIndex.forall { 
      case (p,i) => p.squareDistance(newMeans(i)) <= eta
    }
  }
